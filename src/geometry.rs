use super::{math_3d::*, pixel::Pixel, texture::Texture, transform::*};
...
#[derive(Clone, Debug)]
pub enum MeshType {
    NonIndexed(Vec<Triangle>),
    Indexed(Vec<Vertex>, Vec<u32>),
}

pub struct Primitives;

#[repr(C)]
#[derive(Debug, Copy, Clone, Default, bytemuck::Pod, bytemuck::Zeroable, PartialEq)]
pub struct UV {
    pub u: f32,
    pub v: f32,
    pub w: f32,
...
impl Triangle {
    ...
    pub fn as_vertex_vec(&self) -> Vec<Vertex> {
        self.v.to_vec()
    }
}
...
impl IntoIterator for Triangle {
    type Item = Vertex;
    type IntoIter = std::array::IntoIter<Vertex, 3>;
...
impl PartialEq for Vertex {
    fn eq(&self, v: &Vertex) -> bool {
        approx_equal(self.position.x, v.position.x, 5)
            && approx_equal(self.position.y, v.position.y, 5)
            && approx_equal(self.position.z, v.position.z, 5)
    }
}

impl IntoIterator for &Vertex {
    type Item = Vertex;
    type IntoIter = std::array::IntoIter<Vertex, 1>;
...
impl From<(&[f32; 3], UV)> for Vertex {
    fn from(v: (&[f32; 3], UV)) -> Self {
        Self {
            position: v.0.into(),
            tex_coords: v.1,
            normal: Vector3::default(),
...
impl From<(Vector3, Vector3, Vector3)> for Triangle {
    fn from(v: (Vector3, Vector3, Vector3)) -> Self {
        Self {
            v: [
                Vertex {
                    position: v.0,
...
impl From<[Vector3; 3]> for Triangle {
    fn from(v: [Vector3; 3]) -> Self {
        Self {
            v: [v[0].into(), v[1].into(), v[2].into()],
        }
    }
}

impl From<(Vector3, UV, Vector3, UV, Vector3, UV)> for Triangle {
    fn from(v: (Vector3, UV, Vector3, UV, Vector3, UV)) -> Self {
        Self {
            v: [
                Vertex {
                    position: v.0,
...
impl From<(Vec<Vertex>, Vec<u32>)> for Mesh {
    fn from(vert_index: (Vec<Vertex>, Vec<u32>)) -> Self {
        Mesh {
...
impl Mesh {
    ...
    fn read_buffer<R: std::io::BufRead>(reader: R, has_texture: bool) -> Mesh {
        let mut virts: Vec<Vector3> = vec![];
        let mut texes: Vec<UV> = vec![];
        ...
        for line in reader.lines() {
            ...
            if !l.is_empty() {
                if c[0] as char == 'v' {
                    if c[1] as char == 't' {
                        let uv: UV = Mesh::get_uv_values(&l).into();
                        texes.push(uv);
                    } else if c[1] as char == ' ' {
                        let v: Vector3 = Mesh::get_values(&l).into();
                        virts.push(v);
                    }
                } else if c[0] as char == 'f' {
                    if has_texture {
                        let tokens: Vec<&str> = l.split(' ').collect();
                        let mut virt: [usize; 6] = [1; 6];
                        let mut token_count: usize = 0;
                        for t in tokens.iter() {
                            let v_uv: Vec<&str> = t.split('/').collect();
                            if v_uv.len() > 1 {
                                virt[token_count] = v_uv[0].parse::<usize>().unwrap();
                                token_count += 1;
                                virt[token_count] = v_uv[1].parse::<usize>().unwrap();
                                token_count += 1;
                            }
                        }
                        tris.push(
                        ...
                        let (v1, v2, v3) = Mesh::get_values(&l);
                        tris.push(
                            (
                                virts[(v3 - 1.0) as usize],
                                virts[(v2 - 1.0) as usize],
                                virts[(v1 - 1.0) as usize],
                            )
                                .into(),
                        );
                    }
    ...
    pub fn load_from_string(data: &str, has_texture: bool) -> Mesh {
        Mesh::read_buffer(std::io::BufReader::new(data.as_bytes()), has_texture)
    }

    pub fn load_from_file(path: &str, has_texture: bool) -> Mesh {
        let mesh_file = std::fs::File::open(path).expect("Couldn't open mesh file: {}");
        Mesh::read_buffer(std::io::BufReader::new(mesh_file), has_texture)
    }

    pub fn calculate_normals(&mut self, mode: NormalMode) {
        match &mut self.mesh_type {
            MeshType::NonIndexed(tris) => {
                for tri in tris.iter_mut() {
                    for i in 0..3 {
                        tri.v[i].normal = Vector3::default();
                    }
                }
                match mode {
                    NormalMode::Flat => {
                        for t in tris.iter_mut() {
                            let n = (t.v[2].position - t.v[0].position)
                                .cross(t.v[1].position - t.v[0].position);
                            for i in 0..3 {
                                t.v[i].normal = n;
                            }
                        }
                    ...
                    NormalMode::Shaded => {
                        let v_normals = tris
                            ...
                            .collect::<Vec<(Vector3, Vector3)>>();

                        tris.iter_mut().for_each(|mut t| {
                            for i in 0..3 {
                                let n: Vector3 = v_normals
                                    .iter()
                                    .filter(|n| n.0 == t.v[i].position)
                                    .fold(Vector3::default(), |a, n_i| a + n_i.1);
                                (*t).v[i].normal = n;
                            }
                        });
                        // let tri_slice = tris.clone();
                        // for (i, t) in tris.iter_mut().enumerate() {
                        //     for (n_i, n_tri) in tri_slice.iter().enumerate() {
                        //         'vert: for i in 0..3 {
                        //             if n_tri.v[0] == t.v[i]
                        //                 || n_tri.v[1] == t.v[i]
                        //                 || n_tri.v[2] == t.v[i]
                        //             {
                        //                 (*t).v[i].normal += Vector3::default();
                        //             }
                        //         }
                        //     }
                        // }
            ...
            MeshType::Indexed(verts, indices) => {
                match mode {
                    //This actually doesn't work. We need to duplicate the verts if we want flat shading
                    NormalMode::Flat => {
                        for i in (0..indices.len() - 3).step_by(3) {
                            let normal = (verts[indices[i + 2] as usize].position
                                ...
                                .normal();
                            for o in 0..3 {
                                verts[indices[i + o] as usize].normal = normal;
                            }
                        }
                    }
                    //This accumulates normals for every set of 3 indices. We need to make sure our meshes are triangulated
                    NormalMode::Shaded => {
                        for i in (0..indices.len() - 3).step_by(3) {
                            let normal = (verts[indices[i + 2] as usize].position
                                - verts[indices[i] as usize].position)
                ...
                //     vert.normal = Vector3::default();
                // }
            }
        }
    }

    pub fn apply_transform(&mut self, transform: Transform3) {
        match &mut self.mesh_type {
            MeshType::Indexed(verts, indices) => {
                for v in verts {
                    v.position = ((v.position * transform.scale) * transform.rot) + transform.pos;
                }
            }
            MeshType::NonIndexed(tris) => {
                tris.iter_mut().for_each(|mut t| {
                    t.v.iter_mut().for_each(|mut v| {
                        v.position =
                            ((v.position * transform.scale) * transform.rot) + transform.pos
                    })
                });
            }
        };
    ...
    pub fn make_static_collision(&mut self) {
        // We need to loop through every triangle and see if it shares a normal and an
        // edge(2 vertices) with another. If it does, combine them and simplify
    }

    pub fn is_point_inside(&self, p: Vector3) -> bool {
        false
    }

    /* pub fn closest_point_on_surface(&self, p: Vector3) -> Result<Vector3, &str>{
        let mut dis = f32::MAX;
        let mut v_return = Err("No Collisions");
        for tri in self.tris.iter(){
    let tri_plane = Plane{
                p: tri.p[0],
                n: (tri.p[2] - tri.p[0]).cross(&(tri.p[1] - tri.p[0])).normal()
            };
            if tri_plane.n.dot(&(p - tri_plane.p)) <= 0.0 {
                let i_p = intersect_plane(&tri_plane,
                                          &p, &(p - (tri_plane.n)));
                //Check if I_P in triangle
                let same_side = |p1: Vector3, p2: Vector3, a: Vector3, b: Vector3| -> bool{
                    let cp1 = (b - a).cross(&(p1 - a));
                    let cp2 = (b - a).cross(&(p2 - a));
                    cp1.dot(&cp2) >= 0.0
                };

                if same_side(p, tri.p[0], tri.p[1], tri.p[2]) &&
                    same_side(p, tri.p[1], tri.p[0], tri.p[2]) &&
                    same_side(p, tri.p[2], tri.p[0], tri.p[1]) &&
                    (p - i_p).length() < dis
                {
                    v_return = Ok(i_p);
                    dis = (p - i_p).length();
    ...
    /*pub fn get_collision_correction(&self, p: Vector3, v: Vector3, radius: f32) -> Result<Vector3, &str>{
        let mut v_return = Err("No Collisions");
        let mut old_v = v;
        for tri in self.tris.iter(){
            let tri_plane = Plane{
                p: tri.v[0].position.into(),
                n: {
                    let mut v2: Vector3 = tri.v[2].position.into();
                    let mut v1: Vector3 = tri.v[1].position.into();
                    let mut v0: Vector3 = tri.v[0].position.into();
                    (v2 - v0).cross(&(v1 - v0)).normal()
                }
            };
            if tri_plane.n.dot(&(p - tri_plane.p)) <= 0.0 {
                let (i_p, v_p) = intersect_plane_with_percent(&tri_plane,
                                                              &p, &(p + (v * radius)));
                //Check if I_P in triangle
                let same_side = |p1: Vector3, p2: Vector3, a: Vector3, b: Vector3| -> bool{
                    let cp1 = (b - a).cross(&(p1 - a));
                ...
                if same_side(p, tri.v[0].position.into(),
                             tri.v[1].position.into(),
                             tri.v[3].position.into()) &&

                    same_side(p, tri.v[1].position.into(),
                              tri.v[0].position.into(),
                              tri.v[2].position.into()) &&

                    same_side(p, tri.v[2].position.into(),
                              tri.v[0].position.into(),
                              tri.v[1].position.into())
                    && v_p < 1.0 && v_p > 0.0
                {
                    old_v -= tri_plane.n * tri_plane.n.dot(&v);
                    v_return = Ok(old_v);
                }
            }
        }
    ...
    /*pub fn get_collision_point(&self, p: Vector3, v: Vector3) -> Result<Vector3, &str>{
        let mut v_return = Err("No Collisions");
        for tri in self.tris.iter(){
            let tri_plane = Plane{
                p: tri.p[0],
            ...
            if tri_plane.n.dot(&(p - tri_plane.p)) <= 0.0 {
                ...
                if same_side(i_p, tri.p[0], tri.p[1], tri.p[2]) &&
                    same_side(i_p, tri.p[1], tri.p[0], tri.p[2]) &&
                    same_side(i_p, tri.p[2], tri.p[0], tri.p[1])
                    && v_p <= 1.0 && v_p >= 0.0
                {
                    v_return = Ok(i_p);
                }
    ...
    /* pub fn distance_to_surface(&self, p: Vector3) -> f32{
        let mut dis = f32::MAX;
        if let Ok(point) = self.closest_point_on_surface(p){
            dis = (p - point).length();
        }
        dis
    }*/
    ...
    pub fn get_vert_count(&self) -> u32 {
        match &self.mesh_type {
            MeshType::NonIndexed(tris) => (tris.len() * 3) as u32,
            MeshType::Indexed(verts, indices) => verts.len() as u32,
        }
    }
    ...
    pub fn vertices(&self) -> Vec<Vertex> {
        match &self.mesh_type {
            MeshType::NonIndexed(tris) => tris
                .iter()
                .flat_map(|t| vec![t.v[0], t.v[1], t.v[2]])
                .collect(),
            MeshType::Indexed(verts, indices) => verts.clone(),
        }
    }

    pub fn vert_slice(&self) -> Vec<Vertex> {
        match &self.mesh_type {
            MeshType::NonIndexed(tris) => tris
                .iter()
                .flat_map(|t| vec![t.v[0], t.v[1], t.v[2]])
    ...
    pub fn as_bytes(&self) -> &[u8] {
        match &self.mesh_type {
            MeshType::NonIndexed(tris) => bytemuck::cast_slice(tris.as_slice()),
            MeshType::Indexed(verts, indices) => bytemuck::cast_slice(verts.as_slice()),
        }
    }
}

pub fn intersect_plane(p: &Plane, line_start: &Vector3, line_end: &Vector3) -> Vector3 {
    let np = p.n.normal();
    let dot = -np.dot(p.p);
    let ad = line_start.dot(np);
    let bd = line_end.dot(np);
    let t = (-dot - ad) / (bd - ad);
...
pub fn intersect_plane_percent(p: &Plane, line_start: &Vector3, line_end: &Vector3) -> f32 {
    let np = p.n.normal();
    let dot = -np.dot(p.p);
    let ad = line_start.dot(np);
    let bd = line_end.dot(np);
...
pub fn intersect_plane_with_percent(
    plane: &Plane,
    line_start: &Vector3,
    line_end: &Vector3,
) -> (Vector3, f32) {
    let np = plane.n.normal();
    let dot = -np.dot(plane.p);
    let ad = line_start.dot(np);
    let bd = line_end.dot(np);
    let t = (-dot - ad) / (bd - ad);
    let line_start_to_end = *line_end - *line_start;
...
impl Primitives {
    ...
    pub fn teapot() -> Mesh {
        let teapot_string = r"v -3.000000 1.800000 0.000000
...
f 3022 3021 3001
f 3001 3004 3022";
        Mesh::load_from_string(teapot_string, false)
    }
}
