...
impl Mesh {
    ...
    pub fn load_from_string(data: &str, has_texture: bool) -> Result<Mesh, ()> {
        ...
        for line in data.split('\n').into_iter() {
            ...
            if !c.is_empty() {
                if c[0] as char == 'v' {
                    if c[1] as char == 't' {
                        let uv: UV = Mesh::get_uv_values(line).into();
                        texes.push(uv);
                    } else if c[1] as char == ' ' {
                        let v: Vector3 = Mesh::get_values(line).into();
                        virts.push(v);
                    }
                    ...
                    if has_texture {
                        ...
                        let i: Vector3 = Mesh::get_values(line).into();
                        tris.push(
                            (
                                virts[(i.x - 1.0) as usize],
                                virts[(i.y - 1.0) as usize],
                                virts[(i.z - 1.0) as usize],
                            )
                                .into(),
                        );
                    }
    ...
    pub fn load_from_file(path: &str, has_texture: bool) -> Option<Mesh> {
        let mesh_file =
            std::fs::File::open(path).expect(&format!("Couldn't open mesh file: {}", path));
        let mut virts: Vec<Vector3> = vec![];
        let mut texes: Vec<UV> = vec![];
        let mut tris: Vec<Triangle> = vec![];
        for line in std::io::BufReader::new(mesh_file).lines() {
            let l = line.unwrap();
            let c = l.as_bytes();
            if !l.is_empty() {
                if c[0] as char == 'v' {
                    ...
                    if has_texture {
                        ...
                        let i: Vector3 = Mesh::get_values(&l).into();
                        tris.push(
                            (
                                virts[(i.x - 1.0) as usize],
                                virts[(i.y - 1.0) as usize],
    ...
    pub fn vertices(&self) -> Vec<Vertex> {
        let mut v: Vec<Vertex> = Vec::with_capacity(self.tris.len() * 3);
        for t in &self.tris {
            v.insert(v.len(), t.v[0]);
            v.insert(v.len(), t.v[1]);
            v.insert(v.len(), t.v[2]);
        }
        v
    }
}
...
f 3022 3021 3001
f 3001 3004 3022";
        Mesh::load_from_string(teapot_string, false).unwrap()
    }
}
